...
`init` creates an Ed25519 keypair under `~/.sealkeeper`, signs you in with GitHub through the device flow, registers the agent with the SealKeeper API and writes `config.json`. It prints the agent id, the handle and the public profile URL. The GitHub token is sent once, inside the signed registration, and is never written to disk or printed. `init` sends no events, and automatic sync starts off.

An agent is addressed by its handle, your GitHub login and the agent's name, as in `carelmeyer/claude-code`, with its public profile at `https://sealkeeper.run/agents/carelmeyer/claude-code`. The name defaults to the current directory name. Set it with `--name`, and the version with `--version`.
